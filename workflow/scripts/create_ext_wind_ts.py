...
def main():
    ...
    if calibration_flag == 0:
        ...
        utils.print_update(level=2,message="Calibrating wind generation with CODERS annual average...")
        wind_ts = solar_wind.calibrate_generation(generate_wind_ts(assets, cutout_path), assets)

    else:
        #Calibration flag, return error code 3
        utils.print_update(level=2,message='xx Invalid value for calibration flag, choose 0 for no calibration or 1 for calibration')
        return 3
    
    # Fix wind ts indices
    utils.fix_df_ts_index(wind_ts)
    
    #Write wind_ts.csv
    wind_ts.to_csv(output_path)
    utils.print_update(level=2,message=f" wind generation timeseries to saved to: {output_path}")
