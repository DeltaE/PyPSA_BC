...
def generate_solar_assets(solar_df):
    #Filter down to just the solar panels in BC
    # solar_assets = coders.loc[coders.province.eq('BC') & coders.gen_type_copper.eq('solar')]
    solar_assets = solar_df.rename(columns={'install_capacity_in_mw': 'Install capacity','annual_avg_energy_unit_in_gwh/y': 'CODERS AAG (GWh/y)'}).reset_index().drop(columns=['index'])
    ...
    for code in solar_assets['generation_unit_code'].unique():
        #Get list of indices, [0, 1, 2, ...] for each project
        asset_indexer = solar_assets.loc[solar_assets['generation_unit_code'] == code].reset_index().index.values + 1
    ...
    solar_assets["asset_id"] = solar_assets["generation_unit_code"]
...
def main():
    ...
    solar_assets = coders.loc[(coders['province'] == 'BC')
                          & coders.gen_type_copper.eq('solar')]
