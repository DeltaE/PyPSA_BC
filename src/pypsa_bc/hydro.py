...
def load_reservoir_sites(hydro_sites_file):
    ...
    gdf['cascade_order'] = pd.to_numeric(gdf['cascade_order'], errors='coerce')
    gdf.rename({'latitude':'lat', 'longitude':'lon'}, axis=1,inplace=True)
    utils.print_update(level=2,message="Reservoir sites loaded...")
    return gdf
    ...
    for ind,row in sites.iterrows():
        ...
        for i in range(n_itr):
            ...
            try:
                ...
                exit(1)
            if dfdc.sum() == 0:
                # case where const*inflow(t) > capacity for all t 
                const = const / 1.5 
            else:
...
def create_ror_power(sites_prep, basin_data, cutout, cfg):
    ...
    power_series = utils.fix_df_ts_index(power_series)
    
    power_series.to_csv(cfg["output"]['ror_ps']['fname'])
    utils.print_update(level=2,message="RoR power series saved to: {}".format(cfg["output"]['ror_ps']['fname']))
...
def mean_inflow_calibration(site_inflows, rid, fpath):
    ...
    for idx,row in inflow_stats.iterrows(): # loop over monthly stats 
        ...
        temp_series = pd.concat([s for s in [temp_series, norm_inflow_seg] if not s.empty])
...
def create_cascade_inflow(reservoir_sites, basin_data, cutout, hydro_sites, cfg, method = "mean_inflow_calibrate"):
    ...
    '''
    utils.print_update(level=2, message="Preparing cascade inflows for reservoirs...")
    utils.print_update(level=3, message="Preparing cascade basins...")
    ...
    utils.print_update(level=2, message="calibrating reservoir inflows...")
    ...
    #
    utils.print_update(level=2, message="calibrating snapshot...")
    final_inflows = utils.fix_df_ts_index(final_inflows)

    # power_series = calculate_ror_power(sites, site_inflows)
    reservoir_inflow_save_to=cfg["output"]['reservoir_inflows']['fname']
    final_inflows.to_csv(reservoir_inflow_save_to)
    utils.print_update(level=2, message=f"Reservoir inflows data saved to: {reservoir_inflow_save_to} ")
