...
def calculate_MW(cutout, cluster, type):
    ...
    sites = gpd.GeoDataFrame([[cluster['location'], cluster['longitude'], cluster['latitude'], cluster['facility_installed_capacity']]],
...
def calibrate_generation(ts, assets):
    ...
    # MODIFIED: 2024-10-01 corrections for multi year for adjusting timezone in cutout
    utils.fix_df_ts_index(ts)
    ...
    for component in assets['asset_id'].unique(): #NOTE: component_id -> asset_id (2023-11-15)
        ...
        aag = assets.loc[assets['asset_id'] == component]['facility_average_annual_energy'].values[0] * 1000 #GWh --> MWh
        p = assets.loc[assets['asset_id'] == component]['facility_installed_capacity'].sum()
